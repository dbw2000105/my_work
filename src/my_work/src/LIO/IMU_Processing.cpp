...
void ImuProcess::lic_state_propagate(const MeasureGroup &meas,
  ...
  // if(end_pose_dt > 0)
  // {
  //   cout << ANSI_COLOR_GREEN_BOLD << "End_pose_dt > 0 " << ANSI_COLOR_RESET
  //   << endl;
  // }
...
StatesGroup ImuProcess::imu_preintegration_fast_lio(
    const StatesGroup &state_in, std::deque<sensor_msgs::Imu::ConstPtr> &v_imu,
  ...
  Eigen::Quaterniond q_imu_0 =
      Eigen::Quaterniond(state_inout.rot_end).normalized();
  ...
  for (auto it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++) {
    ...
    // cout <<  std::setprecision(3) << " dt = " << dt << ", acc: " <<
    // acc_avr.transpose()
  ...
  cout << "Vins_pre = " << imu_integration->delta_v.transpose()
       ...
       << ", diff = " << (imu_integration->delta_v - state_inout.vel_end).norm()
       << endl;
