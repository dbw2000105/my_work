...
struct Feature
{
    ...
    // 升序排序的map
    std::map<StateIDType, Eigen::Vector2d, std::less<>,
...
typedef std::map<FeatureIDType, Feature, std::less<>,
...
bool Feature::initializePosition(
    ...
    for (auto &m : observations){
        ...
        auto cam_state_iter = cam_states.find(m.first);//m.first对应的是id longlongint 类型
        if (cam_state_iter == cam_states.end()) //表示，没有找到对应的相机位姿
            continue; //break和continue区别是：break是跳出整个循环，continue是跳出本次循环

        // Add the measurement.
        // 1.1 添加归一化坐标
        measurements.push_back(m.second.head<2>()); //m.second是一个Eigen::Vector4d类型，head<2>表示取前两个元素（u0,v0）
        // measurements.push_back(m.second.tail<2>()); //tail<2>表示取后两个元素(u1,v1)
    ...
    // camera frame.
    // 2. 中心化位姿，提高计算精度
    Eigen::Isometry3d T_c0_w = cam_poses[0]; //把cam_pose这个vector中的第一个元素作为初始的从相机到世界坐标系的变换矩阵
    ...
    do{
        ...
        do{
            // LM算法中的lambda
            Eigen::Matrix3d damper = lambda * Eigen::Matrix3d::Identity(); //3*3的单位矩阵
    ...
    double &w) const
{
    ...
    //W的前两列是旋转矩阵的前两列，最后一列是平移向量，实际上求的是∂h分别对alpha、betla、rho的导数，结果记为W
    ...
    const Eigen::Vector2d &z2, Eigen::Vector3d &p) const
{
    ...
    A(0) = m(0) - z2(0) * m(2);  // 对应第二行，不包括t和d：（m0-z2x*m2）*d+t=0,令前面的系数为A

    // 按照上面推导这里应该是负的但是不影响，因为我们下边b(1)也给取负了
    A(1) = m(1) - z2(1) * m(2);  // 对应第一行

    Eigen::Vector2d b(0.0, 0.0); //这里设的A和b主要是想建立Ax=b的等式，然后用最小二乘法求解x，实际上对应Ad=t
    //这里注意上面公式，md+t是在一个括号里的，外面都要乘一个P2的反对称矩阵，所以t同样要乘。
